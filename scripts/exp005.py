...
def _train_xgb_per_fold(candidates: pl.DataFrame, label: str, fold: str, item_features, df: pl.DataFrame) -> None:
    ...
    train_candidates = make_features(
        ...
        item_features=item_features.rename({"aid": "item"}),
...
def make_features(
    ...
    candidates = (
        candidates.join(item_features, on="item", how="left").join(user_features, on="user", how="left").fill_null(-1)
    )
...
def test():
    def _test_internal_create_candidates() -> pd.DataFrame:
        ...
        candidates = make_features(
            candidates=candidates,
            item_features=item_features.rename({"session": "user", "aid": "item"}),
            user_features=user_features.rename({"session": "user", "aid": "item"}),
        )
